...
def main():
    ...
    try:
        ...
        return 1
    lat, lon = get_ll_from_geocode_response(toponym)
    nearest_drugstore = get_nearest_organization_to_point('аптека', ",".join([lat, lon]))
    drugstore_coord = get_organization_coord(nearest_drugstore)
    drugstore_pointer = ','.join(drugstore_coord) + ',' + 'pm2' + 'gn' + 'l'
    pointer_style = 'pm2'
    pointer_color = 'lb'
    pointer_size = 'l'
    pointer = f'{lat},{lon},{pointer_style}{pointer_color}{pointer_size}'
    pointers = '~'.join([pointer, drugstore_pointer])
    snippet = get_snippet(toponym, nearest_drugstore)
    pprint_snippet(snippet)
